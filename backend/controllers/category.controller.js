...
const addCategoryHandler = async (req, res) => {
  try {
    ...
    if (category) {
      return res
        .status(409)
        .json(new ApiResponse(409, [], "Category With Name Already Exixts"));
    }
    const newCategory = await Category.create(req.body);
    return res
      .status(200)
      .json(new ApiResponse(201, newCategory, "Category Added!"));
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
const getCountHandler = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, category, "Category Count Found!"));
  } catch (error) {
    return res
      .status(500)
      .json(new ApiResponse(500, [], "Internal Server Error"));
