...
const forgetPasswordHandler = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, [], "Reset Link Send To Your Mail!"));
  } catch (error) {
    console.log(error);
    return res
      .status(500)
  ...
  try {
    ...
    try {
      ...
      if (resetTokenData.type === "Admin")
        await Admin.findByIdAndUpdate(tokenData.userId, {
...
const getCountHandler = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, admin, "Admins Count Found!"));
  } catch (error) {
    return res
      .status(500)
      .json(new ApiResponse(500, [], "Internal Server Error"));
...
const resetPasswordHTML = (token, name) => {
  return `<!DOCTYPE html>
    ...
    <body>
        ...
        <p>Best regards,<br>Team XYZ Library</p>
    </body>
    </html>
    `;
};
